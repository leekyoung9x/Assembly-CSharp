...
public class SoundMn
{
  ...
  public void getStrOption()
  {
    ...
    if (Main.isPC)
    {
      ...
      {
        !Char.isPaintAura ? str2 + mResources.aura_off.Trim() : str1 + mResources.aura_off.Trim(),
        !Char.isPaintAura2 ? str2 + mResources.aura_off_2.Trim() : str1 + mResources.aura_off_2.Trim(),
        !GameCanvas.isPlaySound ? str2 + mResources.turnOffSound.Trim() : str1 + mResources.turnOffSound.Trim(),
        mGraphics.zoomLevel <= 1 ? str2 + mResources.x2Screen : str1 + mResources.x1Screen
      };
    }
    else
    {
      string str3 = GameScr.isAnalog != 0 ? str1 + mResources.turnOffAnalog : str2 + mResources.turnOnAnalog;
      if (!GameCanvas.isTouch)
        str3 = GameScr.isPaintChatVip ? str1 + mResources.serverchat_off : str2 + mResources.serverchat_off;
      Panel.strCauhinh = new string[5]
      {
        !Char.isPaintAura ? str2 + mResources.aura_off.Trim() : str1 + mResources.aura_off.Trim(),
        !Char.isPaintAura2 ? str2 + mResources.aura_off_2.Trim() : str1 + mResources.aura_off_2.Trim(),
        !GameCanvas.isPlaySound ? str2 + mResources.turnOffSound.Trim() : str1 + mResources.turnOffSound.Trim(),
        !GameCanvas.lowGraphic ? str2 + mResources.cauhinhthap.Trim() : str1 + mResources.cauhinhthap.Trim(),
        str3
      };
    }
  }
