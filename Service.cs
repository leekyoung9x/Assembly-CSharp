...
public class Service
{
  ...
  public void sendPlayerAttack(MyVector vMob, MyVector vChar, int type)
  {
    try
    {
      Res.outz(">>SEND ATTACT  vMob=" + (object) vMob.size() + "  vChar=" + (object) vChar.size());
      Message message = (Message) null;
      if (type == 0)
        return;
    ...
    {
      Res.err(">>err ATTACT  vMob=" + (object) vMob.size() + "  vChar=" + (object) vChar.size());
    }
  }
  ...
  public void sendOptHat()
  {
    ...
    try
    {
      message.writer().writeByte(Char.myCharz().idHat != (short) -1 ? -1 : 0);
      this.session.sendMessage(message);
    }
    catch (Exception ex)
    {
  ...
  public void new_skill_not_focus(sbyte idTemplateSkill, sbyte dir, short x, short y)
  {
    ...
    {
      message = new Message((sbyte) -45);
      message.writer().writeSByte((sbyte) 20);
