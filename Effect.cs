...
public class Effect
{
  ...
  {
    if (!this.isPaint || Effect.getEffDataById(this.effId) == null)
      return;
    ...
    try
    {
      ...
      else
      {
        if (Effect.getEffDataById(this.effId) == null || Effect.getEffDataById(this.effId).img == null)
          return;
        this.data = this.typeEff != 5 ? Effect.getEffDataById(this.effId).get() : Effect.getEffDataById(this.effId).get(this.c.statusMe);
        if (this.data != null)
        {
          if (!this.isGetTime)
          {
          ...
          switch (this.typeEff)
          {
            ...
            case 5:
              ...
              this.x = this.c.cdir != 1 ? this.c.cx + 15 : this.c.cx - 15;
              this.y = this.c.isMonkey != (sbyte) 0 ? this.c.cy - 35 : this.c.cy - 25;
              if (this.t < this.data.Length)
              {
                ++this.t;
                break;
