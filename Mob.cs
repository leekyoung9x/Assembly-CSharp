...
public class Mob : IMapObject
{
  ...
  public virtual void clearBody() => this.changBody = false;
  ...
  public virtual void paint(mGraphics g)
  {
    ...
    else
    {
      ...
      g.translate(0, -GameCanvas.transY);
      if (Char.myCharz().mobFocus == null || !Char.myCharz().mobFocus.Equals((object) this) || this.status == 1 || this.hp <= 0 || this.imgHPtem == null)
        return;
      int imageWidth = mGraphics.getImageWidth(this.imgHPtem);
      int imageHeight = mGraphics.getImageHeight(this.imgHPtem);
      int w0 = imageWidth * this.per / 100;
      int w = w0;
      if (this.per_tem >= this.per)
      {
        int num1 = imageWidth;
        Mob mob = this;
        int perTem = mob.per_tem;
        int num2 = GameCanvas.gameTick % 6 <= 3 ? this.offset : this.offset++;
        int num3;
        int num4 = num3 = perTem - num2;
        mob.per_tem = num3;
        int num5 = num4;
        w = num1 * num5 / 100;
        if (this.per_tem <= 0)
      ...
      g.setColor(16777215);
      g.fillRect(this.x - (imageWidth >> 1), this.y - this.h - 5, w, 2);
      g.drawRegion(this.imgHPtem, 0, 0, w0, imageHeight, 0, this.x - (imageWidth >> 1), this.y - this.h - 5, mGraphics.TOP | mGraphics.LEFT);
    }
  }
