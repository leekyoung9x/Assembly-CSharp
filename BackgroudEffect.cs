...
public class BackgroudEffect
{
  ...
  public void paintLacay1(mGraphics g, Image img)
  {
    ...
    num = this.typeEff != 15 ? 4 : 4;
    for (int index = 0; index < this.sum; ++index)
    {
      if (index % 3 == 0 && this.x[index] >= GameScr.cmx && this.x[index] <= GameCanvas.w + GameScr.cmx && this.y[index] >= GameScr.cmy && this.y[index] <= GameCanvas.h + GameScr.cmy && BackgroudEffect.PIXEL <= mGraphics.getImageHeight(img) && BackgroudEffect.PIXEL * this.frame[index] < img.getHeight())
        g.drawRegion(img, 0, BackgroudEffect.PIXEL * this.frame[index], img.getWidth(), BackgroudEffect.PIXEL, 0, this.x[index], this.y[index], 0);
    }
  }
