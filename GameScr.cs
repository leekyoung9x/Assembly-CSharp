...
public class GameScr : mScreen, IChatable
{
  ...
  public void doSelectSkill(Skill skill, bool isShortcut)
  {
    ...
    else if (Char.myCharz().isUseSkillSpec())
    {
      Res.outz(">>>use skill spec: " + (object) skill.template.id);
    ...
    else
    {
      ...
      else if (Char.myCharz().statusMe != 14)
      {
        ...
        else if (GameScr.isAnalog != 1)
        {
          ...
          g.setColor(16777215);
          g.fillRect(GameScr.xHP + 9, GameScr.yHP + 10 + (num2 == 0 ? 0 : 20 - num2) - 6, 22, num2 == 0 ? 20 : num2);
          g.drawImage(mScreen.keyTouch != 10 ? GameScr.imgHP1 : GameScr.imgHP2, GameScr.xHP, GameScr.yHP - 6, 0);
          mFont.tahoma_7_red.drawString(g, string.Empty + (object) GameScr.hpPotion, GameScr.xHP + 20, GameScr.yHP + 15 - 6, 2);
        }
        else
        {
          g.setColor(9670800);
          ...
          g.setColor(16777215);
          g.fillRect(GameScr.xHP + 10, GameScr.yHP + 10 + (num2 == 0 ? 0 : 20 - num2) - 6, 20, num2 == 0 ? 18 : num2);
      ...
      {
        Skill[] skillArray = Main.isPC ? GameScr.keySkill : (!GameCanvas.isTouch ? GameScr.keySkill : GameScr.onScreenSkill);
        ...
        int num5 = !Main.isPC ? (!GameCanvas.isTouch ? skillArray.Length : this.nSkill) : skillArray.Length;
        for (int index = 0; index < num5; ++index)
        {
          ...
          else if (!GameCanvas.isTouch)
          {
            ...
            g.setColor(11152401);
            g.fillRect(GameScr.xSkill + GameScr.xS[index] + 2, GameScr.yS[index] - 10 + 8, 20, 10);
            mFont.tahoma_7_white.drawString(g, strArray4[index], GameScr.xSkill + GameScr.xS[index] + 12, GameScr.yS[index] - 10 + 6, mFont.CENTER);
          }
  ...
  public void showWinNumber(string num, string finish)
  {
    ...
    try
    {
      ...
      {
        this.winnumber[index] = (int) short.Parse(str[index].ToString());
  ...
  public void paintPhuBanBar(mGraphics g, int x, int y, int w)
  {
    ...
    int y1 = y + frameHeight + mGraphics.getImageHeight(GameScr.imgBall) / 2 + 2;
    ...
    int y)
  {
    if (GameScr.imgBall == null)
      return;
    int h0 = mGraphics.getImageHeight(GameScr.imgBall) / 2;
    for (int index = 0; index < maxLife; ++index)
    {
      int num = 0;
      if (index < lifeTeam1)
        num = 1;
      g.drawRegion(GameScr.imgBall, 0, num * h0, mGraphics.getImageWidth(GameScr.imgBall), h0, 0, x1 - index * (h0 + 1), y, mGraphics.VCENTER | mGraphics.HCENTER);
    }
    for (int index = 0; index < maxLife; ++index)
    {
      int num = 0;
      if (index < lifeTeam2)
        num = 1;
      g.drawRegion(GameScr.imgBall, 0, num * h0, mGraphics.getImageWidth(GameScr.imgBall), h0, 0, x2 + index * (h0 + 1), y, mGraphics.VCENTER | mGraphics.HCENTER);
    }
  }
  ...
  public static void addEffectEnd(
    int type,
    int subtype,
    int typePaint,
    int x,
  ...
  public static void addEffectEnd_Target(
    int type,
    int subtype,
    int typePaint,
    Char charUse,
    ...
    short range)
  {
    GameScr.addEffect2Vector(new Effect_End(type, subtype, typePaint, charUse, target, levelPaint, timeRemove, range));
  }
  ...
  private void paint_xp_bar(mGraphics g)
  {
    ...
    for (int index = 1; index < 10; ++index)
      g.fillRect(index * num, GameCanvas.h - 2, 1, 2);
  }
}
